...
当前节点同时运行着 **Chrome Relay**（claw-relay）和 **browser-harness**。两者都基于 CDP，但定位不同：

| | **browser-harness** | **Chrome Relay（claw-relay）** |
|---|---|---|
| 语言 | Python | Node.js |
| 架构 | CLI 直连 Chrome CDP | MCP Server 代理 + 自管 Chrome |
| Chrome 来源 | 连接已有 Chrome（任意开了 remote debugging 的） | claw-relay 自己启动专属 Chrome 窗口 |
| 接口层 | 裸 CDP，AI 自主操作 | 结构化 Action API（navigate/click/type/snapshot/screenshot） |
| 权限控制 | 无，依赖 Chrome remote debugging 端口权限 | 内置 auth token、rate limit、site blocklist |
| AI 适应性 | **AI 可自己写 agent_helpers.py** | 固定 action set，AI 无法扩展底层能力 |
| domain-skills | 有（社区维护的网站专项知识） | 无 |
| 审计日志 | 无内置 | 有（audit.jsonl）|
| Chrome Extension | 无 | 可选 extension，劫持真实 Chrome |
| 安装复杂度 | 需 clone + uv + 配置 Chrome | `npx claw-relay` 一条命令 |

### 怎么选

- **需要 AI 自主适应未知网站** → browser-harness（AI 自己写辅助代码）
- **需要权限管控/审计/多人共用** → Chrome Relay（有 token + blocklist）
- **需要操作真实已登录 Chrome** → Chrome Relay（extension 模式）
- **纯 WSL2 环境** → 两者都受限（IP 问题），优先 browser-harness + Cloud Browser

### 共存方案

当前节点 Chrome Relay 在 `ws://127.0.0.1:9333`，browser-harness 连 OpenClaw Chromium 在 `http://127.0.0.1:18800`。可以同时跑，互不干扰。
