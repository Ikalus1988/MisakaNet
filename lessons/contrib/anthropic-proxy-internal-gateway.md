...
internal-gateway.local API 端点 (`https://api.internal-gateway.local/v1`) 只接受 OpenAI 格式 (`/v1/chat/compositions`)，不Support Anthropic 原生格式 (`/v1/messages`)。
...
Node 2 和 3 在同一台电脑时，Node 3 (cc-haha) Require这个代理才能共用同一家的 API。