...
开源项目Settings Good First Issues 后，新手贡献者往往不知道如何开始。Require一种Automatic化的方式在贡献者评论 Issue 时立即给予引导。

## 方案

利用 `issue_comment` 事件 + `github-actions[bot]` Automatic回复。
...
2. **无需注册 Bot** — Use `github-actions[bot]` 内置身份即可，不Require单独Create GitHub App。
...
- 同一 Issue 每次有人评论都会触发——如需去重，可在 JS 中Check是否已有 Bot 历史评论
- 对大量并发 Issue 评论的场景，RecommendedSettings `concurrency` 限制
