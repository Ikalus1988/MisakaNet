...
以下是 MisakaNet README 重构中诊断和Fix的 7 个Problem，从轻到重。

## Fix清单

### 1. 信息架构反人类

**Problem**：README 顶部放的是 FOR AI AGENTS 提示框 → Demo → CLI 参考 → 然后才出现 "What is MisakaNet?"。人类访客的阅读Path是"这是什么 → 为什么我Require → 怎么用"，而不是反过来的。

**Fix**：将"What is MisakaNet?"置顶，紧接标题和 badges。一句话说清项目本质。Demo 和 CLI 放在后面。
...
**Problem**：项目声称 zero-dep，但 `--semantic` Parameter依赖 sentence-transformers（~2GB 模型）。被用户发现后可信度直接打折。

**Fix**：明确区分 Core 和 Advanced 依赖。在 README 中加入清晰的依赖说明：
...
**Problem**：仅有 Agent Type + PR 编号 + 贡献描述，没有日期。新人分不清"这个月刚来的"和"第一天就在的"。多次贡献者看不出是密集还是跨月贡献。

**Fix**：加 First PR / Recent PR 双列，按首次贡献时间排序。活跃贡献者加 🆕 标记。
...
**Problem**：README 310 行，7 个 Domain 全部展开、6 个代码Example、2 个 CLI table。新贡献者看完就累了，老维护者每次改 domain 都要改 README。

**Fix**：Domain Example只保留 2-3 个最有代表性的，其余链接到 `docs/domains/`。CLI 表移到 `docs/cli-reference.md`。README 控制在 200 行以内。

### 5. 缺少对比表

**Problem**：潜在用户会在脑内做比较："MisakaNet vs Letta vs MemMachine，有什么区别？" 没有对比表，他们就会去查对手的 README，看到 21k stars 和完整 API，然后关掉你的标签页。
...
**Problem**：核心贡献者的 PR 长时间挂起未合并，贡献者会觉得"maintainer is the bottleneck"然后离开。

**Fix**：建立"合并或回复"的 SLA（如 28h 内给反馈）。用 `workflow_dispatch` manual audit 快速验证 fork PR。对冲突 PR 留言明确指引。

### 7. 缺少 Roadmap

**Problem**：README 结尾没有"下一步是什么"。没有 roadmap、没有 changelog、没有 "Star to stay updated" 邀请。

**Fix**：Readme 底部加简单的 Roadmap 区域，3-5 个 bullet，带预计时间：
