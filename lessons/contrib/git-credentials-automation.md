...
脚本Automatic执行 `git push` 时弹出用户名密码输入框，导致Automatic化流程卡住。

## 根因

Git Default用交互式凭证管理器（`manager` 或 `askpass`），非 TTY 环境下无法输入。
...
# 2. 或Use git-credential-oauth（更安全）
...
- `~/.git-credentials` 是明文——Ensure `.gitignore` 忽略它或用Environment variable
- Token Require `repo` 和 `workflow` 权限
