...
在 CodeWhale 的 Agent 模式中，`exec_shell` 工具不可用。Require执行 `git push` 时，不能直接用 shell Command。

## 根本Cause

CodeWhale Agent 模式的工具集不含 `exec_shell`。有两条替代Path：
...
```python
task_create(
    prompt="在 /mnt/c/Users/hp/MisakaNet 执行... <具体Command>",
    mode="yolo",
    allow_shell=True,
    auto_approve=True,
...
不加 `trust_mode=True` 时，`exec_shell` 会在审批队列里等待约 2-3 分钟，最终也会Via。

## 重要：操作前先Verify仓库身份

另一个反复出现的Error：把 **Agent-Medici** 当成 **MisakaNet** 来改。
...
git -C /mnt/c/Users/hp/MisakaNet remote -v       # Verify这是你要改的 repo
```
