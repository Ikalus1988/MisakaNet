...
Require在 DeepSeek TUI 中在线操作飞书云文档（docx），Via MCP 协议暴露飞书 API。
...
飞书 App ID / App Secret 不应硬编码在 server.py 中，应Via DeepSeek TUI 的 mcp.json env 字段注入：
...
MCP Server Start时读取Environment variable，缺失则 fail-fast 退出。
...
| API 类别 | 可用 | 所需权限 |
|----------|------|----------|
| docx 云文档 | ✅ Create/读取/编辑 | docx:document（Default自带） |
| IM 消息发送 | ✅ 发送到群/用户 | im:message（Default自带） |
| IM 群列表 | ✅ 列出群信息 | im:chat（Default自带） |
| 消息接收（WS） | ❌ 需事件订阅 | im.message.receive_v1 事件 |
| 消息历史查询 | ❌ 需额外权限 | im:message.group_msg |
| 云盘/File | ❌ 未开通 | drive:drive |
| Wiki 知识库 | ❌ 未开通 | wiki:wiki |
| 文档评论 | ❌ 未开通 | docs:document.comment:read |
| 联系人 | ❌ 未开通 | contact:contact.base:readonly |

**关键**: 即使 docx Create API 可用（code=0），读取Other用户Create的文档也可能返回 1770032 forBidden——Require文档所有者将机器人Add为协作者。

### 4. 飞书 Block API 特性

- PATCH Update block 内容可用（update_text_elements）
- DELETE block Interface返回 404（飞书 docx API 可能不Support直接Delete）
- 替代方案：PATCH 将内容清空
- 批量追加 children 每批Recommended ≤40 blocks
...
deepseek mcp list        # View注册的 MCP 服务
```

MCP Configuration保存在 `~/.deepseek/mcp.json`。
...
- 不Support sheet（电子表格）和 wiki（知识库）
- 文档评论Require额外权限（docs:document.comment:read）
- 消息接收Require事件订阅Configuration + 发布应用Version
