...
- 查询「TCP 设定方法」时，回答来源中包含Other品牌的操作手册
...
### 2. 品牌过滤条件不足
检索层已有的品牌过滤代码仅在查询中包含目标品牌关键词时触发 —— 但日常巡检的Problem通常不写品牌名。

### 3. File名检测漏洞
File名不包含明确品牌标记的文档被遗漏（例如 kap05_1_*.pdf 实际是竞品文档，但File名未体现品牌）。
...
- 先扫描全量分类，再分批写入
- 由于 ChromaDB 的持久化特性，Update后需重建 BM25 缓存

### 第二步：补充遗漏品牌文档

File名分类会遗漏不含品牌名的文档。Via内容关键词搜索补漏：
...
1. ChromaDB 的 `query()` 返回最新 metadata，但 BM25 索引会缓存旧 metadata —— Update后必须Delete BM25 缓存File并重建
2. File名正则分类会遗漏不含品牌名的文档，Require用内容语义搜索补充
