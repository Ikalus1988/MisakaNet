---
{"title": "注册链路设计 — Worker 只Create Issue，其余交给 Workflow", "domain": "devops", "tags": ["registration", "worker", "register", "github-actions", "feishu", "fallback"], "domain_expert": "unknown"}
---

## Background

MisakaNet 节点注册Require一条对国内外用户都通畅的链路。最初 Worker 既Create Issue 又读写 counter.json，导致 Worker 和 register.yml 双重自增、竞态、Worker 权限过大等Problem。

## 根本Cause

旧 Worker 的设计缺陷：

1. **双重自增**：Worker Via GitHub API 读写 counter.json，同时 register.yml（由 Worker Create的 Issue 触发）也读写 counter.json → 同一个注册加两次
2. **Worker 权限过大**：Require `contents: write`（读写 counter.json），增加了 Token 泄露风险
...
Worker 职责精简为**只Create Issue**，不 touch counter.json：

```
用户表单 → Worker → Create Issue → 返回 issue_url + issue_number
                                   ↓
...
```
🌐 注册节点暂时不可达
📋 有 GitHub 账号？→ 点此Create Issue（template=register.yml）
    ↓
register.yml Automatic处理（30 秒完成）
```
...
同时加 `workflow_dispatch` 兜底——如果Automatic触发失败可以Manual补注册。
...
```
📧 无 GitHub 账号？
填 Agent Type + 联系方式 → 提交 → 飞书群通知管理员
                                       ↓
                                管理员ManualCreate Issue
                                       ↓
                                register.yml Automatic处理
                                       ↓
                                管理员回复用户节点号
```
...
Note：飞书群机器人 Webhook 只能**发**消息，不能收。消息会推送到Configuration了该 Webhook 的飞书群。

### 前端轮询：⏳ 分配中 → ✅ 已分配

注册成功后，前端每 10 秒轮询 GitHub API Check Issue 评论：
...
```
⏳ 分配中  →  ✅ 已分配
Misaka10051 → Misaka10051（Verify）
⏳ 等待刷新 →（移除）
```
...
```bash
# 注册链路设计 — Worker 只Create Issue，其余交给 Workflow
...
- 前端轮询的 CSS 选择器要和成功 UI 的 class 名匹配，否则轮询到了也不会Update UI