...
**Note**：`BrowserHarness` 类在文档Example中出现，但CurrentVersion（0.1.0）不存在这个类。正确用法是 `browser-harness` CLI。
...
- **Browser Use Cloud**：带住宅代理的云浏览器，Default US 代理，可选 195+ 国家
- **Stealth Browsers**：自定义 Chromium 内核，Automatic绕过 DataDome/Kasada/Cloudflare
- **CAPTCHA**：Stealth Browsers 免费Automatic解决 Turnstile/reCAPTCHA
...
- 表单Automatic填写
- 多标签页协同操作
- Require AI 自主适应的复杂导航任务

⚠️ **需额外Configuration：**
- 国内 App（微信/小红书/抖音）：Require Browser Use Cloud 住宅代理
- Require登录的网站：Require profile-sync 同步 cookie
- 高安全站点（银行等）：Require Stealth Browsers

## browser-harness vs Chrome Relay（OpenClaw 内置）

Current节点同时Run着 **Chrome Relay**（claw-relay）和 **browser-harness**。两者都基于 CDP，但定位和Interface方式完全不同。
...
|---|---|---|
| 语言 | Python | Node.js |
| Interface方式 | CLI + Python 执行环境，exec 一段 Python 代码 | WebSocket + 结构化 JSON Action API |
| Chrome 来源 | 连接已有 Chrome（任意开了 remote debugging 的） | claw-relay 自己Start专属 Chrome 窗口，或连接已有 Chrome |
...
| 批量操作 | 无原生 batch，需Manual循环 | Support batch + stopOnError 原子性 |
| Install复杂度 | 需 clone + uv + Configuration Chrome | `npx claw-relay` 一条Command |
...
| 场景 | 推荐 |
|------|------|
| Require AI 自主适应未知网站，AI 自己写辅助代码 | **browser-harness** |
| Require权限管控/审计/多人共用 | **Chrome Relay**（token + blocklist）|
| Require操作真实已登录 Chrome | **Chrome Relay**（extension 模式）|
| 批量操作Require原子性（遇错停止） | **Chrome Relay**（batch + stopOnError）|
| WSL2 环境 | 两者都受限（IP Problem），优先 browser-harness + Cloud Browser |
...
可以同时跑，互不干扰。Chrome Relay 也Support连外部 Chrome（需 `--no-chrome` + Configuration `cdpUrl`）。
## Verification
...
- Install文档：https://github.com/browser-use/browser-harness/blob/main/install.md
