...
netsh advfirewall firewall add rule 只是允许本机 WSL2 虚拟机访问 Windows 主机的端口。流量Path在**单台电脑内部**：
...
powershell `netsh advfirewall firewall show rule name="wcferry"` 可View规则详情。

## 场景

WSL2 + Windows 混合开发，任何Require WSL 访问 Windows 端口的情况（RAG API、微信机器人等）。
