...
Hub Configuration了 `im.message.receive_v1` 和 `p2p_card_action.trigger` 回调，但从未收到飞书消息。
...
在 `hermes_hub.py` 的 `start()` 方法中Add：
...
async def start(self):
    ...
    # 必须调用，否则 WS 从未Start
    await self.feishu_ws_client.start()  # ← Add这行
    await self._register_handlers()
    self._start_background_tasks()
```

已在 commit `56f690b` 中Fix。
...
```
grep "长连接已Start" ~/.hermes/logs/hermes_hub_new.log
```
