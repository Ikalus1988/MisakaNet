...
`git push` / `git fetch` 持续超时或报 TLS 握手Error：
...
- `/etc/hosts` Modify后即时生效，无需重启
- 如果 Git Configuration了代理（`git config --global http.proxy`），优先排查代理
- hosts 条目在 DNS 正常的环境下不会造成冲突，可作为常驻方案
- 不同运营商/地区可达的 IP 可能不同，Recommended现场扫描
