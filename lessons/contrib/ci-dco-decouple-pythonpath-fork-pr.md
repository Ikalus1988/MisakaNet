...
1. **ModuleNotFoundError** — `pr-checks.yml` 用 `pip install -e .` Install项目包，但 fork PR 分支的 `pyproject.toml` Use了 `setuptools.backends._legacy` 构建后端（不兼容 runner Default setuptools Version），导致 `pip install -e .` 崩溃。

2. **DCO gate 锁死测试** — workflow 将所有测试Steps挂在 `if: steps.dco.outputs.dco_passed == 'true'` 后面。一旦 commits 缺 `Signed-off-by:`，全部测试被跳过，无法区分"代码质量Problem"和"CI ConfigurationProblem"。
...
原理：将仓库根Directory加入 `PYTHONPATH`，Python 直接发现顶层包（如 `misakanet/`），无需 PEP 517 构建。规避了 `pyproject.toml` 后端不兼容Problem，且执行速度更快（秒级 vs 构建数秒）。

### 2. 解耦 DCO 与测试执行

移除所有测试Steps的 `if: steps.dco.outputs.dco_passed == 'true'` 门禁。DCO 依然作为独立CheckRun和报告，但不阻塞测试。最终报告同时包含 DCO 状态和测试结果：
...
### 3. coverage 阈Value适配小 PR

Settings `--cov-fail-under=20` 而非 70，避免小改动 PR（如单FileFix）被覆盖率门禁卡住。大改动 PR 自然会有更高覆盖率。

### 4. Manual审计 workflow（workflow_dispatch）

对于已经跑过 CI 的 fork PR，`gh run rerun` 复用旧 workflow 定义，无法获取Update后的 workflow File。Solution是Create一个独立的 `manual-audit.yml`：
...
- 40 个测试全部Via，无 ModuleNotFoundError
- DCO 失败时测试依然Run并报告真实结果
- Manual dispatch 可在 2 分钟内对任意 fork PR 执行审计

## 进阶：工业级 DCO Bot 留言模板

DCO 失败时，可以让 `github-actions[bot]` Automatic在 PR 下发布结构化Fix指引（非人工复制粘贴）。

### 方案

在 `dco-check.yml` 中新增一个 step（仅在 DCO Check失败时触发）：
...
1. **不用 heredoc 渲染模板** — YAML `|` 块和 shell heredoc 的缩进要求冲突。改用 `{ echo ... } > file` 逐行构建，规避缩进Problem。
2. **模板Variable注入** — Use GitHub Actions `env:` 传入Variable，shell 中 `echo "$VAR"` 展开。对Require动态替换的部分用双引号，纯文本部分用单引号。
3. **身份策略** — `GH_TOKEN: ${{ secrets.GITHUB_TOKEN }}` 以 `github-actions[bot]` 身份留言，不消耗人类账号。
4. **workflow_dispatch 兜底** — PR 的首次触发Use旧 workflow，重跑也用旧定义。加 `workflow_dispatch` + `pr_number` 输入可强制Use main 分支的最新 workflow File。
5. **fork PR 的 HEAD 获取** — 在 `workflow_dispatch` 中Require用 `refs/pull/$NUM/head` 而非直接 fetch SHA，因为 fork 的 commit 不在 origin remote 中。

## 关键教训

1. `pip install -e .` 对 fork PR 不可靠（pyproject.toml 可能不兼容）— PYTHONPATH 更稳健
2. DCO 门禁应该报告而非阻塞 — 让贡献者同时看到所有失败Cause
3. `gh run rerun` Use旧 workflow 定义 — Require `reopened` PR 事件或 `workflow_dispatch` 才能激活新 workflow
4. `reopened` PR 事件可能被 path filter 阻挡 — 不设 path filter 或UseManual dispatch
