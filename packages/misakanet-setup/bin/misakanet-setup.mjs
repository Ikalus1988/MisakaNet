...
/** The voice player + cues: shipped in the tarball, or read from a repo checkout. */
function locateVoiceDir() {
  for (const dir of [join(PKG_ROOT, 'voice'), join(PKG_ROOT, '..', '..', 'integrations', 'agent-autostart')]) {
    if (existsSync(join(dir, 'voice-hook.mjs'))) return dir;
  }
  return null;
}
  ...
  if (has('--voice')) {
    ...
    if (!source) {
      ...
      if (!DRY) {
        ...
        copyFileSync(join(source, 'voice-hook.mjs'), join(destDir, 'voice-hook.mjs'));
        for (const cue of readdirSync(source).filter((f) => f.endsWith('.mp3'))) {
          copyFileSync(join(source, cue), join(destDir, cue));
        }
      }
