...
function createEnv(seed = {}) {
  ...
  return {
    // Synthetic and per-run: a literal here is indistinguishable from a credential to a
    // scanner, and hol-guard's plugin-scanner reads whole files — comments included
    // (tests/test_scanner_secret_patterns.py caught exactly that, 2026-09-15).
    MCP_TOKEN: testToken('protocol-negotiation'),
