...
// Exit on the next tick rather than through `process.exit(0)` *in* the finally above.
//
// The old shape was:
//
//     } finally { process.exit(0); }
//     setTimeout(() => process.exit(0), 3000).unref();   // ← unreachable, every run
//
// `process.exit` ends the process before the backstop line is ever evaluated, so the safety net
// for "a detached player kept a handle open" had never run — dead code that read like protection.
// (Found by the 2026-09-18 setup review; verified by reading the tail of this file, which is also
// why the package's `voice/voice-hook.mjs` copy is regenerated by `prepack`.)
//
// Why not just `process.exitCode = 0` with no exit call at all: the debug line above is written with
// `writeFileSync(1, …)` precisely so it cannot be truncated, and this process must not stay alive
// waiting on a spawned player — the agent is waiting for the hook to return. `setImmediate` keeps
// stdout draining for one tick while still ending the process deterministically, and the timer is
// the real backstop for a handle the spawn left behind.
setImmediate(() => {
  setTimeout(() => process.exit(0), 3000).unref();
  // If the loop is otherwise empty, `unref`ed timers do not hold it open, so Node exits by itself
  // within this tick — the timer only matters when something else is still keeping us alive.
  process.exitCode = 0;
});
