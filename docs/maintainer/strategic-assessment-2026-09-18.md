...
**推广复盘时不要用错指标**（2026-09-18 实测）：

- `https://api.npmjs.org/downloads/point/last-month/@misaka-net/misakanet-setup` 会给出 **0**——它的
  窗口（08-13…09-11）早于首发（09-14T11:41Z）。要看区间数据，用 `downloads/range/<起>:<止>/<包名>`，
  并注意非零日可能只有发布当天（本轮的 916 次下载只有 2 个非零日）。
- `/api/counter` 的 10303 是**匿名 node 数**，不是用户数：不带 `client_id` 时每次调用都新建一个 node
  （`AGENTS.md` §3.3），而 0.5.4 之前的安装器还没有 `--client-id`，所以"安装数 ≈ node 增长"这种推断
  在旧版本上会被放大。
- GitHub 的 `traffic/clones`（14 天 23326）包含 CI、镜像与爬虫，不能当用户数。
